...
const generateStockCheckCode = async () => {
  ...
  if (lastStockCheck && lastStockCheck.check_code) {
    const lastNumber = parseInt(lastStockCheck.check_code.substring(prefix.length));
    if (!isNaN(lastNumber)) {
...
const createStockCheck = async (req, res) => {
  try {
    ...
    if (!check_date) {
      return res.status(400).json({ message: "Thời gian kiểm kho là bắt buộc" });
    }

    if (!items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ message: "Danh sách hàng hóa kiểm kho là bắt buộc" });
    }

    // Kiểm tra user là manager
    const user = await User.findById(userId);
    if (!user || user.role !== "MANAGER") {
      return res.status(403).json({ message: "Chỉ Manager mới được tạo phiếu kiểm kho" });
    }
    ...
    if (store.owner_id.toString() !== userId) {
      return res.status(403).json({ message: "Bạn chỉ có thể tạo phiếu kiểm kho trong cửa hàng của mình" });
    }

    // Kiểm tra và xác thực từng sản phẩm trong danh sách
    for (const item of items) {
      if (!item.product_id || item.book_quantity === undefined || item.actual_quantity === undefined) {
        return res.status(400).json({ message: "Mỗi sản phẩm phải có đầy đủ product_id, book_quantity và actual_quantity" });
      }
      ...
      if (!item.cost_price || item.cost_price <= 0) {
        return res.status(400).json({ message: `Sản phẩm ${product.name} chưa có giá vốn hoặc giá vốn không hợp lệ` });
      }

      if (!item.price || item.price <= 0) {
        return res.status(400).json({ message: `Sản phẩm ${product.name} chưa có giá bán hoặc giá bán không hợp lệ` });
      }
    }
...
const getStockCheckById = async (req, res) => {
  try {
    ...
    // Kiểm tra quyền truy cập
    const user = await User.findById(userId);
    if (user.role === "MANAGER" && stockCheck.store_id.owner_id.toString() !== userId) {
      return res.status(403).json({ message: "Bạn không có quyền truy cập phiếu kiểm kho này" });
    }
...
const updateStockCheck = async (req, res) => {
  try {
    ...
    if (stockCheck.store_id.owner_id.toString() !== userId) {
      return res.status(403).json({ message: "Bạn chỉ có thể cập nhật phiếu kiểm kho trong cửa hàng của mình" });
    }

    // Không cho phép cập nhật phiếu đã cân bằng
    if (stockCheck.status === 'Đã cân bằng' && status !== 'Đã cân bằng') {
      return res.status(400).json({ message: "Không thể cập nhật phiếu kiểm kho đã cân bằng" });
    }

    // Kiểm tra và xác thực items nếu được cung cấp
    if (items) {
      if (!Array.isArray(items) || items.length === 0) {
        return res.status(400).json({ message: "Danh sách hàng hóa phải là mảng và không được trống" });
      }
      ...
      for (const item of items) {
        ...
        if (product.store_id.toString() !== stockCheck.store_id._id.toString()) {
          return res.status(400).json({ message: `Sản phẩm ${product.name} không thuộc cửa hàng này` });
        }
...
const deleteStockCheck = async (req, res) => {
  try {
    ...
    if (stockCheck.store_id.owner_id.toString() !== userId) {
      return res.status(403).json({ message: "Bạn chỉ có thể xóa phiếu kiểm kho trong cửa hàng của mình" });
    }
