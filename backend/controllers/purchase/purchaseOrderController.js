...
const generateOrderCode = async (storeId) => {
  try {
    ...
    if (
      lastOrder &&
      lastOrder.purchase_order_code &&
      lastOrder.purchase_order_code.startsWith("NH")
    ) {
...
const createPurchaseOrder = async (req, res) => {
  try {
    ...
    if (!user || user.role !== "MANAGER") {
      return res
        .status(403)
        .json({ message: "Chỉ Manager mới được tạo đơn nhập hàng" });
    }
    ...
    if (!supplier) {
      return res.status(404).json({
        message: "Nhà cung cấp không tồn tại hoặc không thuộc cửa hàng này",
      });
    }
    ...
    if (supplier.status !== "đang hoạt động") {
      return res
        .status(400)
        .json({ message: "Nhà cung cấp đã ngừng hoạt động" });
    }
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!item.product_id) {
        return res
          .status(400)
          .json({ message: `Sản phẩm thứ ${i + 1}: product_id là bắt buộc` });
      }
      ...
      if (!product) {
        return res.status(404).json({
          message: `Sản phẩm thứ ${
            i + 1
          } không tồn tại hoặc không thuộc cửa hàng này`,
        });
      }
      ...
      if (!item.quantity || item.quantity <= 0) {
        return res
          .status(400)
          .json({ message: `Sản phẩm thứ ${i + 1}: Số lượng phải lớn hơn 0` });
      }
    ...
    if (purchase_order_code) {
      ...
      if (existingOrder) {
        return res
          .status(400)
          .json({ message: "Mã đơn hàng này đã tồn tại trong cửa hàng" });
      }
    ...
    if (!validStatuses.includes(orderStatus)) {
      return res.status(400).json({
        message:
          "Trạng thái không hợp lệ. Chỉ chấp nhận: 'phiếu tạm', 'đã nhập hàng', 'đã hủy'",
      });
    }
...
const getPurchaseOrderById = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Bạn không có quyền truy cập đơn nhập hàng này" });
    }
    ...
    if (user.role === "STAFF") {
      ...
      if (
        employee.store_id.toString() !== purchaseOrder.store_id._id.toString()
      ) {
        return res
          .status(403)
          .json({ message: "Bạn không có quyền truy cập đơn nhập hàng này" });
...
const updatePurchaseOrder = async (req, res) => {
  try {
    ...
    if (!user || user.role !== "MANAGER") {
      return res
        .status(403)
        .json({ message: "Chỉ Manager mới được cập nhật đơn nhập hàng" });
    }

    // Tìm đơn nhập hàng
    const purchaseOrder = await PurchaseOrder.findById(orderId).populate(
      "store_id",
      "owner_id"
    );
    if (!purchaseOrder) {
    ...
    if (
      purchaseOrder.status === "đã nhập hàng" ||
      purchaseOrder.status === "đã hủy"
    ) {
      return res.status(400).json({
        message: "Không thể cập nhật đơn hàng đã nhập hàng hoặc đã hủy",
      });
    }
    ...
    if (items !== undefined && Array.isArray(items)) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!product) {
          return res
            .status(404)
            .json({ message: `Sản phẩm thứ ${i + 1} không tồn tại` });
        }

        if (!item.quantity || item.quantity <= 0) {
          return res.status(400).json({
            message: `Sản phẩm thứ ${i + 1}: Số lượng phải lớn hơn 0`,
          });
        }
      ...
      updateData.total_amount = calculateTotalFromItems(
        validatedItems,
        productMap
      );
    }
    ...
    if (paid_amount !== undefined) {
      if (paid_amount < 0) {
        return res
          .status(400)
          .json({ message: "Số tiền đã trả không được âm" });
      }
      updateData.paid_amount = parseFloat(paid_amount);
    }

    // Kiểm tra nếu trạng thái thay đổi thành "đã nhập hàng" để cập nhật stock
    const isStatusChangedToImported =
      status === "đã nhập hàng" && purchaseOrder.status !== "đã nhập hàng";
...
const deletePurchaseOrder = async (req, res) => {
  try {
    ...
    if (!user || user.role !== "MANAGER") {
      return res
        .status(403)
        .json({ message: "Chỉ Manager mới được xóa đơn nhập hàng" });
    }
    ...
    if (purchaseOrder.status !== "đã nhập hàng") {
      return res
        .status(400)
        .json({ message: "Chỉ có thể hủy đơn hàng đã nhập hàng" });
    }
