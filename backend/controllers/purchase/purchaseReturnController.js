...
const updateStockOnReturn = async (items, storeId, operation) => {
  ...
  for (const item of items) {
    const stockChange =
      operation === "decrease" ? -item.quantity : item.quantity;
    bulkOps.push({
      updateOne: {
...
const createPurchaseReturn = async (req, res) => {
  ...
  if (!supplier_id || !items || items.length === 0) {
    return res
      .status(400)
      .json({ message: "Nhà cung cấp và danh sách sản phẩm là bắt buộc." });
  }
  ...
  try {
    ...
    if (!user || user.role !== "MANAGER") {
      return res
        .status(403)
        .json({ message: "Chỉ Manager mới được tạo phiếu trả hàng." });
    }
    ...
    if (!store || store.owner_id.toString() !== userId) {
      return res
        .status(403)
        .json({
          message: "Bạn không có quyền tạo phiếu trả hàng trong cửa hàng này.",
        });
    }
    ...
    if (!supplier) {
      return res
        .status(404)
        .json({ message: "Nhà cung cấp không tồn tại trong cửa hàng này." });
    }
    ...
    if (purchase_order_id) {
      ...
      if (!purchaseOrder) {
        return res
          .status(404)
          .json({ message: "Đơn nhập hàng không tồn tại trong cửa hàng này." });
      }
      ...
      if (purchaseOrder.supplier_id.toString() !== supplier_id) {
        return res
          .status(400)
          .json({
            message: "Nhà cung cấp không khớp với đơn nhập hàng được chỉ định.",
          });
      }
    }
    ...
    if (purchase_return_code) {
      ...
      if (existingReturn) {
        return res
          .status(400)
          .json({
            message: `Mã trả hàng '${purchase_return_code}' đã tồn tại trong cửa hàng này.`,
          });
      }
    } else {
    ...
    if (purchaseOrder) {
      const orderProductIds = purchaseOrder.items.map((item) =>
    ...
    for (const item of items) {
      ...
      if (!product) {
        return res
          .status(400)
          .json({
            message: `Sản phẩm với ID ${item.product_id} không tồn tại trong cửa hàng.`,
          });
      }
      if (!item.return_price || item.return_price < 0) {
        return res
          .status(400)
          .json({
            message: `Giá trả lại cho sản phẩm ${product.name} là bắt buộc và phải lớn hơn hoặc bằng 0.`,
          });
      }
...
const getPurchaseReturnById = async (req, res) => {
  try {
    ...
    if (!purchaseReturn) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy phiếu trả hàng." });
    }
...
const updatePurchaseReturn = async (req, res) => {
  ...
  try {
    ...
    if (!user || user.role !== "MANAGER") {
      return res
        .status(403)
        .json({ message: "Chỉ Manager mới được cập nhật phiếu trả hàng." });
    }
    ...
    if (purchaseReturn.status === "đã trả hàng") {
      return res
        .status(400)
        .json({
          message: "Không thể cập nhật phiếu đã ở trạng thái 'đã trả hàng'.",
        });
    }
    ...
    if (
      updates.status &&
      updates.status === "đã trả hàng" &&
      purchaseReturn.status !== "đã trả hàng"
    ) {
      await updateStockOnReturn(
      ...
      for (const item of updates.items) {
        ...
        if (!item.return_price || item.return_price < 0) {
          return res
            .status(400)
            .json({
              message: `Giá trả lại cho sản phẩm ${product.name} là bắt buộc.`,
            });
        }
...
const deletePurchaseReturn = async (req, res) => {
  ...
  try {
    ...
    if (!user || user.role !== "MANAGER") {
      return res
        .status(403)
        .json({ message: "Chỉ Manager mới được hủy phiếu trả hàng." });
    }
    ...
    if (purchaseReturn.status === "đã trả hàng") {
      return res
        .status(400)
        .json({
          message: "Không thể hủy phiếu đã ở trạng thái 'đã trả hàng'.",
        });
    }
    ...
    res
      .status(200)
      .json({
        message: "Phiếu trả hàng đã được hủy thành công.",
        purchaseReturn: updatedReturn,
      });
  } catch (error) {
    res.status(500).json({ message: "Lỗi server: " + error.message });
