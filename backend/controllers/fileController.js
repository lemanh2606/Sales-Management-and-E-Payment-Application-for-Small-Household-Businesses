...
const uploadFile = async (req, res) => {
  try {
    ...
    if (!storeId || !req.file) {
      return res
        .status(400)
        .json({ message: "Thiếu storeId hoặc file upload" });
    }
    ...
    if (disallowedExt.includes(fileExt)) {
      return res
        .status(400)
        .json({
          message: `File có đuôi ".${fileExt}" không được phép tải lên!`,
        });
    }
    ...
    await logActivity({
      ...
      } đã tải lên tệp mới "${newFile.name}" (${
        newFile.extension
...
const getFilesByStore = async (req, res) => {
  try {
    ...
    const files = await File.find({ storeId })
    ...
    res
      .status(500)
      .json({ message: "Lỗi lấy danh sách file", error: err.message });
  }
};
...
const deleteFile = async (req, res) => {
  try {
    ...
    await logActivity({
      ...
      description: `Người dùng ${
        req.user.username || req.user.email
      } đã xoá tệp "${file.name}" khỏi cửa hàng ${
