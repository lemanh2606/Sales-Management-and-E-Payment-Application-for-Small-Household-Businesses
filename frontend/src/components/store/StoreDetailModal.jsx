...
import { Modal, Card, Row, Col, Space, Button, Tag, Avatar, Divider, Typography, Tooltip, QRCode } from "antd";
import {
  EditOutlined,
  DeleteOutlined,
  ShopOutlined,
...
export default function StoreDetailModal({ open, onClose, store, onEdit, onSelect, onDelete, onRestore }) {
  ...
  const confirmDeleteStore = async () => {
    const result = await Swal.fire({
      ...
      text: `Bạn có chắc chắn muốn xóa cửa hàng "${store.name}" không?`,
      icon: "warning",
  ...
  const confirmRestoreStore = async () => {
    const result = await Swal.fire({
      title: "Xác nhận khôi phục cửa hàng?",
      text: `Bạn có muốn khôi phục cửa hàng "${store.name}" không?`,
      icon: "question",
      showCancelButton: true,
      ...
      <div>
        ...
        >
          ...
          <Row gutter={24} align="middle" style={{ position: "relative", zIndex: 1 }}>
                ...
                <Space size={8} wrap>
                  ...
                  {googleMapsUrl && (
                    <Tooltip title="Xem trên Google Maps">
                      <Button type="link" href={googleMapsUrl} target="_blank" icon={<LinkOutlined />} style={{ color: "#fff", padding: 0 }}>
                        Maps
                      </Button>
                    </Tooltip>
                  )}
            ...
            {googleMapsUrl && (
              <Col flex="none">
                <QRCode value={googleMapsUrl} size={96} color="#1f1f1f" style={{ background: "#fff", padding: 4, borderRadius: 8 }} />
              </Col>
            )}
          </Row>
        </div>
            ...
            <Col xs={24} md={14}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "minmax(160px, 30%) 1fr" }}>
                  ...
                  {[
                    ...
                    },
                    {
                      ...
                      label: "Ngày tạo",
                      content: <Text>{store.createdAt ? new Date(store.createdAt).toLocaleString("vi-VN") : "-"}</Text>,
                    },
                  ].map((row, idx) => (
              ...
              {store.description && (
                <>
                  ...
                  >
                    <Paragraph ellipsis={{ rows: 5, expandable: true, symbol: "Xem thêm" }} style={{ margin: 0, fontSize: 14, lineHeight: 1.7 }}>
              ...
              >
                <Space direction="vertical" size={12} style={{ width: "100%" }}>
                  ...
                  <Space direction="vertical" size={8}>
                    ...
                    {googleMapsUrl && (
                      <Button icon={<LinkOutlined />} type="default" href={googleMapsUrl} target="_blank" style={{ borderRadius: 8 }}>
                        Mở Google Maps
                      </Button>
                    )}
                  </Space>
