...
export default function SidebarItem({ item, collapsed = false }) {
  ...
  return (
    <div className="relative group text-gray-700">
      ...
      {/* submenu chỉ hiện khi open,  cái nào style zalo thì vẫn giữ nguyên, cái nào sang tab mới thì check newTab ở sidebar */}
      {!collapsed && hasChildren && open && (
        <div className="pl-6 mt-1 space-y-1">
          {" "}
          {/* 👉 FIX: Thêm space-y-1 cho khoảng cách submenu mượt */}
          {item.children.map((child) =>
            child.newTab ? (
              <a
                ...
                className="block py-2 px-3 rounded-lg text-gray-600 text-sm hover:bg-green-100 hover:text-green-700 transition"
              >
                {child.name}
              </a>
            ) : (
      ...
      {/* flyout cho chế độ collapsed, cái nào style zalo thì vẫn giữ nguyên, cái nào sang tab mới thì check newTab ở sidebar */}
      {collapsed && hasChildren && (
                  ...
                  {item.children.map((ch) =>
                    ch.newTab ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-3 py-2 text-base hover:bg-gray-50 block text-green-600 transition-colors"
                      >
                        {ch.name}
                      </a>
                    ) : (
