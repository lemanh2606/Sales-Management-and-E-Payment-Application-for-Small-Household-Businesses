...
const ProcessExpiredPage = () => {
  ...
  const [createSupplierModalVisible, setCreateSupplierModalVisible] =
    useState(false);
  ...
  const fetchWarehouses = async () => {
    try {
      const res = await axios.get(`${apiUrl}/warehouses/${storeId}`, {
        headers,
      });
  ...
  const fetchSuppliers = async () => {
    try {
      const res = await axios.get(
        `${apiUrl}/suppliers/store/${storeId}?limit=100`,
        { headers }
      );
  ...
  const handleSelectItems = (selectedRows: any[]) => {
    ...
    const uniqueWarehouses = [
      ...new Set(selectedRows.map((it) => it.warehouse_id).filter(Boolean)),
    ];
    ...
    const uniqueSupplierIds = [
      ...new Set(selectedRows.map((it) => it.supplier_id).filter(Boolean)),
    ];

    if (uniqueSupplierIds.length === 1) {
      const supId = uniqueSupplierIds[0];
      const representativeItem = selectedRows.find(
        (it) => it.supplier_id === supId
      );
      ...
      if (representativeItem) {
        ...
        if (currentSupId !== supId) {
          const contactPerson =
            representativeItem.supplier_contact ||
            representativeItem.contact_person ||
            "";
    ...
    try {
      ...
      const payload = {
        ...
        items: selectedItems.map((item) => ({
          ...
          description: `Hết hạn ${dayjs(item.expiry_date).format(
            "DD/MM/YYYY"
          )}`,
        })),
      };
      ...
      await axios.post(
        `${apiUrl}/stores/${storeId}/inventory-vouchers/process-expired`,
        payload,
        { headers }
      );
  ...
  const columns = [
    ...
    { title: "NCC", dataIndex: "supplier_name", key: "supplier_name" },
    {
      ...
      render: (_: any, r: any) => {
        ...
        return days < 0 ? (
          <Tag color="red">Hết hạn {Math.abs(days)} ngày</Tag>
        ) : (
  ...
  return (
    <Layout>
        ...
        title={
          <Space>
            ...
            <span style={{ fontWeight: 700 }}>
              Danh sách hàng hết hạn / cận date (30 ngày)
            </span>
          </Space>
        }
        extra={
      ...
      >
        <Table
          ...
          rowSelection={{
            ...
            selectedRowKeys: selectedItems.map(
              (i) => `${i._id || i.id}-${i.batch_no || "none"}`
            ), // ✅ Phải khớp với rowKey
          }}
          loading={loading}
      ...
      <Modal
        title={
          mode === "DISPOSE"
            ? "Tạo Phiếu Tiêu Hủy Hàng Hóa"
            : "Tạo Phiếu Trả Hàng Nhà Cung Cấp"
        }
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <Space direction="vertical" style={{ width: "100%" }}>
            <Alert
              message={`Bạn đang chọn: ${selectedItems.length} mặt hàng để ${
                mode === "DISPOSE" ? "TIÊU HỦY" : "TRẢ LẠI"
              }`}
              description={
                <ul>
                  {selectedItems.slice(0, 3).map((i, idx) => (
                    <li key={idx}>
                      <b>{i.name}</b> - Lô: {i.batch_number} - SL: {i.quantity}
                    </li>
                  ))}
                  {selectedItems.length > 3 && (
                    <li>... và {selectedItems.length - 3} mặt hàng khác ...</li>
                  )}
                </ul>
              }
              type="info"
              ...
              >
                ...
                {mode === "RETURN" ? (
                  <Space.Compact style={{ width: "100%" }}>
                    ...
                    <Button
                      icon={<PlusOutlined />}
                      onClick={() => setCreateSupplierModalVisible(true)}
                    />
                  </Space.Compact>
                ) : (
            ...
            >
              <TextArea
                rows={3}
                placeholder="Ví dụ: Tiêu hủy do vi khuẩn xâm nhập, Trả hàng cho NCC ABC theo thỏa thuận bảo hành..."
              />
            </Form.Item>
          </Space>
          ...
          <div style={{ textAlign: "center" }}>
            <Text type="secondary" italic style={{ fontSize: 12 }}>
              * Phiếu này sẽ được ghi sổ tự động và cập nhật tồn kho tức thời.
              Vui lòng kiểm tra kỹ trước khi bấm xác nhận.
            </Text>
          </div>
        </Form>
      </Modal>
      ...
      >
        <div style={{ padding: 20 }}>
          ...
          <Text type="secondary" style={{ fontSize: 12 }}>
            * Nhà cung cấp này sẽ được thêm tạm thời vào danh sách để xử lý
            phiếu.
          </Text>
        </div>
      </Modal>
