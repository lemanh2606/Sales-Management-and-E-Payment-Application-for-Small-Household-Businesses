...
const EndOfDayReport: React.FC = () => {
  ...
  const getPeriodKey = (): string => {
    ...
    switch (periodType) {
      case "day":
        return vnDate.format("YYYY-MM-DD");
      case "month":
        return vnDate.format("YYYY-MM");
      case "quarter":
        return `${vnDate.year()}-Q${Math.floor(vnDate.month() / 3 + 1)}`;
      case "year":
        return vnDate.format("YYYY");
      default:
        return vnDate.format("YYYY-MM-DD");
    }
  };
  ...
  const handleExportExcel = async () => {
    try {
      ...
      message.error({
        content: err.response?.data?.message || "Lỗi xuất Excel",
        key: "export",
      });
    }
  };
  ...
  const handleExportPDF = async () => {
    try {
      ...
      message.error({
        content: err.response?.data?.message || "Lỗi xuất PDF",
        key: "export",
      });
    }
  };
  ...
  return (
    <div className="premium-layout">
      <Space direction="vertical" size="large" style={{ width: "100%", backgroundColor: "white" }}>
        {/* HEADER & FILTERS */}
        <Card className="glass-card">
          <Row gutter={[24, 24]} align="middle">
            ...
            <Col xs={12} lg={4}>
              ...
              <Select value={periodType} onChange={handlePeriodChange} style={{ width: "100%" }} size="large">
            ...
            <Col xs={24} lg={8} style={{ textAlign: "right" }}>
              <Space>
                <Button size="large" icon={<UndoOutlined />} onClick={() => loadReport(selectedDate, periodType)}>
                  Làm mới
                </Button>
                <Button
                  type="default"
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} lg={10}>
            <Card
              ...
              style={{ borderRadius: 12, height: "100%", border: "1px solid #8c8c8c" }}
            >
        ...
        <Card
          ...
          style={{ marginTop: 16, borderRadius: 12, border: "1px solid #8c8c8c" }}
        >
          <Table
