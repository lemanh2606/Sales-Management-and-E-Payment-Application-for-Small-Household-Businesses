...
export default function VerifyOtpPage() {
    ...
    const handleResend = async () => {
        setError("");
        try {
            // We reuse register endpoint to resend OTP: create user must fail if exists,
            // so for resend you might need a dedicated endpoint. As a simple approach,
            // call register again only if server supports resend; otherwise implement backend resend endpoint.
            // Here we call a /resend-otp hypothetical endpoint. If backend doesn't have it, remove this.
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-6">
            <div className="w-full max-w-md p-8 bg-white rounded-3xl shadow-xl border-t-4 border-green-500">
                ...
                <p className="text-gray-600 mb-3">OTP đã gửi tới: <b>{email}</b></p>
                <p className="text-gray-500 mb-4">Hết hạn sau: <span className="font-mono">{formatTime(timer)}</span></p>
                ...
                <form onSubmit={handleVerify}>
                    ...
                    <InputField label="OTP" name="otp" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="Nhập mã 6 chữ số" />
                    <Button type="submit" className="w-full mt-3">Xác nhận</Button>
                </form>

                <div className="mt-4 flex justify-between items-center text-sm">
                    <button onClick={handleResend} disabled={timer > 0} className="text-green-600 hover:underline disabled:text-gray-300">
                        Gửi lại OTP
                    </button>
