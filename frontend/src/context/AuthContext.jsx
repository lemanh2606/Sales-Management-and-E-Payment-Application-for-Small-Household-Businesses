...
export const AuthProvider = ({ children }) => {
  ...
  const [managerSubscriptionExpired, setManagerSubscriptionExpiredState] =
    useState(() => {
    ...
    const interceptor = apiClient.interceptors.response.use(
      ...
      async (error) => {
        ...
        if (
          ...
          console.warn(
            "⚠️ 403 Forbidden - Not token related:",
            errorData.message
          );
        }
      ...
      if (userData?.role === "STAFF") {
        try {
          ...
          if (staffStoreId) {
            ...
            console.warn(
              "No store found for STAFF, redirecting to select-store"
            );
