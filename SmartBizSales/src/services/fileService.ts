...
class FileService {
  ...
  private async downloadFileMobile(
    ...
    try {
      ...
      if (blob._data && blob._data.blobId) {
        ...
        console.log(
          "🔄 Processing React Native Blob with blobId:",
          blob._data.blobId
        );
  ...
  private async readReactNativeBlob(blob: any): Promise<ArrayBuffer> {
    try {
      ...
      console.log(
        `📊 Successfully converted blob to ArrayBuffer, size: ${arrayBuffer.byteLength} bytes`
      );
  ...
  async downloadFromUrl(
    ...
    try {
      ...
      const output: any = await File.downloadFileAsync(
        url,
        destination,
        fileName
      );
