...
const InventoryReportScreen: React.FC = () => {
  ...
  // ===== STATE =====
  const [activeTab, setActiveTab] = useState<"realtime" | "variance">(
    "realtime"
  );
  ...
  const [vPeriodType, setVPeriodType] = useState<"month" | "quarter" | "year">(
    "month"
  );
  ...
  const periodDisplay = useMemo(() => {
    if (vPeriodType === "month")
      return `Tháng ${dayjs(vDate).format("MM/YYYY")}`;
    if (vPeriodType === "quarter") {
      const q = Math.floor(vDate.getMonth() / 3) + 1;
  ...
  const fetchVarianceReport = async (isRefresh: boolean = false) => {
    ...
    try {
      const res = await apiClient.get<VarianceResponse>(
    ...
    return (
      <View style={styles.itemCard}>
        <View style={styles.itemHeaderRow}>
          <View style={{ flex: 1 }}>
            <Text
              style={[styles.itemName, item.lowStock && { color: "#ef4444" }]}
              numberOfLines={2}
            >
              {item.productName}
            </Text>
  ...
  const renderVarianceItem = ({ item }: { item: VarianceDetail }) => {
    ...
    const changeColor =
      netChange > 0 ? "#10b981" : netChange < 0 ? "#ef4444" : "#64748b";
      ...
      <View style={styles.itemCard}>
        <View style={styles.itemHeaderRow}>
          ...
          <View style={{ alignItems: "flex-end" }}>
            ...
            <Text style={[styles.stockValue, { color: "#2563eb" }]}>
              {item.endingStock}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.varianceGrid}>
          ...
          <View style={styles.varianceCell}>
            ...
            <Text style={[styles.varianceValue, { color: "#10b981" }]}>
              +{item.importQty}
            </Text>
          </View>
          <View style={styles.varianceCell}>
            <Text style={styles.varianceLabel}>Xuất</Text>
            <Text style={[styles.varianceValue, { color: "#ef4444" }]}>
              -{item.exportQty}
            </Text>
          </View>
          <View style={styles.varianceCell}>
            ...
            <Text
              style={[styles.varianceValue, { color: "#f59e0b", fontSize: 13 }]}
            >
              {formatCurrency(item.periodCOGS)}
            </Text>
          </View>
        </View>
      </View>
    ...
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.centerText}>
          Vui lòng chọn cửa hàng trước khi xem báo cáo.
        </Text>
      </View>
    );
  }
      ...
      <LinearGradient colors={["#10b981", "#3b82f6"]} style={styles.header}>
        <View style={styles.headerTopRow}>
          ...
          <View style={styles.headerButtons}>
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                activeTab === "realtime"
                  ? fetchRealtimeReport(false)
                  : fetchVarianceReport(false)
              }
              disabled={loading}
            >
              {loading ? (
        ...
        <View style={styles.tabContainer}>
          <TouchableOpacity
            style={[
              styles.tabBtn,
              activeTab === "realtime" && styles.tabBtnActive,
            ]}
          ...
          >
            <Ionicons
              name="cube-outline"
              size={16}
              color={activeTab === "realtime" ? "#10b981" : "#fff"}
            />
            <Text
              style={[
                styles.tabText,
          ...
          <TouchableOpacity
            style={[
              styles.tabBtn,
              activeTab === "variance" && styles.tabBtnActive,
            ]}
          ...
          >
            <Ionicons
              ...
              size={16}
              color={activeTab === "variance" ? "#10b981" : "#fff"}
            />
            <Text
              style={[
                styles.tabText,
                activeTab === "variance" && styles.tabTextActive,
              ]}
            >
              Biến thiên
            </Text>
        ...
        <View style={styles.filterBar}>
          <View style={styles.filterRow}>
            <TouchableOpacity
              style={[
                styles.filterChip,
                vPeriodType === "month" && styles.filterChipActive,
              ]}
            ...
            >
              <Text
                style={[
                  ...
                  vPeriodType === "month" && styles.filterChipTextActive,
                ]}
              >
                Tháng
              </Text>
            ...
            <TouchableOpacity
              style={[
                styles.filterChip,
                vPeriodType === "quarter" && styles.filterChipActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.filterChipText,
                  vPeriodType === "quarter" && styles.filterChipTextActive,
                ]}
              >
                Quý
              </Text>
            ...
            <TouchableOpacity
              style={[
                styles.filterChip,
                vPeriodType === "year" && styles.filterChipActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.filterChipText,
                  vPeriodType === "year" && styles.filterChipTextActive,
                ]}
              >
                Năm
              </Text>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={() =>
              activeTab === "realtime"
                ? fetchRealtimeReport(true)
                : fetchVarianceReport(true)
            }
            colors={["#10b981"]}
          />
        }
          ...
          <View style={styles.summarySection}>
            <View style={styles.chartWrap}>
              <PieChart
                data={[
                  ...
                  {
                    value: Math.max(0, reportData.summary.totalValue * 0.2),
                    color: "#d1fae5",
                  },
                ]}
                radius={35}
            ...
            <View style={styles.summaryChipsRow}>
              ...
              <View
                style={[
                  styles.summaryChip,
                  lowStockCount > 0 && { backgroundColor: "#fef2f2" },
                ]}
              >
                <Text
                  style={[
                    styles.summaryChipLabel,
                    lowStockCount > 0 && { color: "#ef4444" },
                  ]}
                >
                  Tồn thấp
                </Text>
