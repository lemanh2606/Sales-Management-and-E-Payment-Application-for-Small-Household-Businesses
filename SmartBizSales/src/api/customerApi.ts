...
export const softDeleteCustomer = async (id: string): Promise<{ message: string }> => {
 ...
 */
export const restoreCustomer = async (id: string): Promise<{ message: string }> => {
    const response = await apiClient.put<ApiResponse<{ message: string }>>(`/customers/${id}/restore`);
    return unwrap<{ message: string }>(response);
};
...
export const getCustomersByStore = async (
    ...
    return {
        customers: Array.isArray((data as any)?.customers) ? (data as any).customers : [],
        total: Number((data as any)?.total ?? ((data as any)?.customers?.length ?? 0)),
        ...
        count: Array.isArray((data as any)?.customers) ? (data as any).customers.length : 0,
    };
};
...
export const bulkCreateCustomers = async (
    ...
    const response = await apiClient.post<ApiResponse<{ created: number; errors: any[] }>>("/customers/bulk", {
        customers,
    });
 ...
 */
export const exportCustomers = async (storeId: string): Promise<ArrayBuffer> => {
