...
export const updateProductBatch = async (
  ...
  const response = await apiClient.put<any>(`/products/${productId}/batch`, {
    batchIndex,
    ...data,
  });
  return response.data;
...
export const getProductGroupsByStore = async (
  storeId: string
): Promise<{ productGroups: any[] }> => {
  if (!storeId) {
    throw new Error("Thiếu storeId khi lấy danh sách nhóm sản phẩm");
  }

  try {
    const response = await apiClient.get<{ productGroups: any[] }>(
...
export const bulkUpdateProducts = async (
  ...
  const response = await apiClient.patch<{
    message: string;
    updatedCount: number;
  }>("/products/bulk-update", { productIds, data });
  return response.data;
};
